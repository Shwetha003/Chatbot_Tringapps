...
from app.models.chat import Document, ChatMessage
...
async def process_crawl_request(
    ...
    try:
        ...
        if not url_match:
            raise HTTPException(
                status_code=400,
                detail="Provided text is not a valid URL."
            )
        ...
        existing_entry = (
            db.query(Document)
            .filter(Document.source_url == target_url)
            .first()
        )
        ...
        if not existing_entry:
            ...
            scraped_text = await scrape_to_pure_text(
                target_url,
                db,
                db_doc.document_id
            )
            ...
            scraped_text = "URL already exists in database"
...
async def process_chatbot_message(
    ...
    try:
        ...
        if chat_file and chat_file.filename.endswith(".pdf"):
            ...
            page_report = await PDFService.route_pdf_pages(
                file_bytes
            )
            ...
            with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
                ...
                for page in page_report:
                    ...
                    if route == "text_heavy":
                        ...
                        page_data = await PDFService.process_hybrid_page(
                            file_bytes,
                            page_num
                        )
            ...
            pdf_markdown = DocumentConverter.to_markdown(
                collected_pages
            )

            pdf_json = DocumentConverter.to_json(
                collected_pages
            )
            ...
            document = crud_chat.create_document_record(
                db=db,
            ...
            chunk_count = await PDFService.save_pdf_chunks(
        ...
        if not payload.message and chat_file:
            payload.message = (
                "Analyze this uploaded document, summarize its contents, "
                "and tell me what actions I can take."
            )

        bot_reply = await llm_service.generate_bot_reply(
            db=db,
            payload=payload,
            context=pdf_markdown
        )
